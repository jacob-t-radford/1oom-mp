...
extern bool game_send_fleet_retreat(struct game_s *g, player_id_t owner, planet_id_t from, planet_id_t dest, const shipcount_t ships[NUM_SHIPDESIGNS], const uint8_t shiptypes[NUM_SHIPDESIGNS], uint8_t numtypes);
...
extern uint8_t game_fleet_get_speed(const struct game_s *g, const struct fleet_enroute_s *r, planet_id_t pfrom, planet_id_t pto);
extern void game_fleet_redirect(struct game_s *g, struct fleet_enroute_s *r, planet_id_t pfrom, planet_id_t pto);
